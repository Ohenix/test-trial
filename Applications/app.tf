...
#Security group for app to communicate in and out
...
# target group for alb to register our application instances or task, also do health checks
...
# this will create a subdomain of the application, so the it could be reached from the load balancer. 
  condition {
    host_header {
